...
const COLUMNS_PATH = path.resolve(process.cwd(), 'data/columns.json');
const DB_PATH = path.resolve(process.cwd(), 'data/products.db');
  ...
  try {
    ...
    if (fs.existsSync(COLUMNS_PATH)) {
      try {
        ...
        console.log(`📋 Loaded ${columnsData.length} columns from columns.json`);
    ...
    console.log(`📋 JSON file not found, reading columns from database schema`);
    
    // Check if database exists and has data
    if (!fs.existsSync(DB_PATH)) {
      console.log(`📋 Database doesn't exist, returning empty columns`);
    ...
    if (tableInfo.length === 0) {
      console.log(`📋 No products table found, returning empty columns`);
      return NextResponse.json({ success: true, columns: [] });
    }

    // If database is empty (no products), return empty columns
    if (productCount.count === 0) {
      console.log(`📋 Database is empty (0 products), returning empty columns`);
      return NextResponse.json({ success: true, columns: [] });
    }
    ...
    const columns = tableInfo
      ...
      .map((col: any) => {
        ...
        if (col.name.toLowerCase().includes('price') || col.name.toLowerCase().includes('quantity') || col.name.toLowerCase().includes('amount')) {
          columnType = 'number';
    ...
    console.log(`📋 Read ${columns.length} columns from database schema`);
    return NextResponse.json({ success: true, columns });
  } catch (e: any) {
